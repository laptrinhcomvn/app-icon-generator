...
- (void)dropComplete:(NSString *)filePath {
    ...
    inputPath = filePath;
    outputPath = [[filePath stringByDeletingLastPathComponent] stringByAppendingPathComponent:@"Icons"];
    ...
    lbOutputPath.stringValue = outputPath;
}
- (IBAction)btnGenerateClicked:(id)sender {
    ...
    if([fm createDirectoryAtPath:outputPath withIntermediateDirectories:YES attributes:nil error:&error]){
        ...
        NSRunAlertPanel(@"Error", @"%@", nil, nil, nil, error.description);
    }
}
